...
public class DriverBigData extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		inputPath = new Path(args[1]);	// Input data of the application
		outputDir = new Path(args[2]);  // Output folder of the first job
		...
		if (job.waitForCompletion(true) == true) {
			...
			// One single instance of the reducer class for the second job
			numberOfReducersJob2 = 1;   

			// Set path of the input file/folder (if it is a folder, the job
			// reads all the files in the specified folder) for this second job.
			// The input folder of the second job is equal to the output 
			// folder of the first job
